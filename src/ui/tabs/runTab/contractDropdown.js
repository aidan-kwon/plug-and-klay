...
class ContractDropdownUI extends React.Component {
  constructor (props) {
    ...
    this.logCallback = global.terminal.logHtml
  ...
  componentDidUpdate (prevProps: Readonly<P>, prevState: Readonly<S>, snapshot: SS) {
    ...
    if (updatedAt !== prevProps.updatedAt) {
      this.setState({
        ...
        selected: this.state.selected || (this.props.list.length > 0 ? this.dropdownLogic.getSelectedContract(this.props.list[0].name, this.props.list[0].compiler) : undefined)
      })
    }
  }
  ...
  updateSelectedContract = (e) => {
    const name = e.target.value || (e.name || '')
    ...
    return <div className={css.container} data-id="contractDropdownContainer">
        ...
        <select onLoad={ this.setInputParamsPlaceHolder } onChange={ this.updateSelectedContract } ref={ ref => this.selectContractNames = ref } className={`${css.contractNames} custom-select`} disabled={ this.state.compFails }>
          {
            contracts.length > 0 && contracts.map(contract => {
              return <option value={ contract.name } compiler={ contract.compilerFullName }>{contract.name} - {contract.fileName}</option>
            })
          }
        </select>
        {
          (contracts.length < 1 || compFails) && <i
            ...
            className={`m-2 ml-3 fas fa-times-circle ${css.errorIcon}`}/>
        }
      ...
      <div>
        <div ref={ ref => this.createPanel = ref } className={ css.deployDropdown }>
          ...
          { (selected !== undefined) && <MultiParamManager lookupOnly={false} funABI={ selected.getConstructorInterface() } clickCallBack={ this.deployClickCallback } inputs={ selected.getConstructorInputs() } title={ 'Deploy' }
                                           evmBC={ selected.bytecodeObject } isDeploy={ true } /> }
          { (selected !== undefined) && <div className="mt-2 d-flex align-items-start">
            <IpfsCheckbox checked={ this.state.ipfsChecked }/>
            <label
        ...
        <div ref={ ref => this.orLabel = ref } className={ css.orLabel } mt-2>or</div>
  ...
  changeCurrentFile = (currentFile) => {
    ...
    if (/.(.abi)$/.exec(currentFile)) {
      ...
      this.compFails.style.display = 'none'
      contractNames.appendChild(<option>(abi)</option>)
  ...
  async createInstance (selectedContract, args) {
    console.log(selectedContract, args)
    ...
    var continueCb = (error, continueTxExecution, cancelCb) => {
      if (error) {
        ...
        modalDialog('Gas estimation failed', <div>Gas estimation errored with the following message (see below).
    ...
    const confirmationCb = this.getConfirmationCb(modalDialog, confirmDialog)
      ...
      // return this.blockchain.createContract(selectedContract, args, continueCb, promptCb, confirmationCb, finalCb)

    // this.blockchain.transactionContextAPI.getGasLimit((err, limit) => {
    //   if (err) {
    //     return console.log(err)
    //   }
    //
    //   this.blockchain.transactionContextAPI.getAddress((err, address) => {
    //     if (err) {
    //       return console.log(err)
    //     }
    //
    //     return this.deploy(selectedContract, args, {
    //       gasLimit: limit,
    //       from: address,
    //       gasPrice: '0',
    //       value: '0',
    ...
    // try {
    //   contractMetadata = await this.dropdownLogic.deployMetadataOf(this.blockchain, selectedContract.name)
    // } catch (error) {
    ...
    if (selectedContract.isOverSizeLimit()) {
      return modalDialog('Contract code size over limit', <div>Contract creation initialization returns data with length of more than 24576 bytes. The deployment will likely fails. <br/>
  ...
  loadFromAddress = () => {
    if (!this.state.selected) {
      return modalDialogCustom.alert('contract not compiled')
    }
    ...
    this.dropdownLogic.loadContractFromAddress(address, selectedContract,
      (cb) => {
