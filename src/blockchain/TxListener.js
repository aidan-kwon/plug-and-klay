...
const txFormat = require('remix-lib').execution.txFormat
const txHelper = require('remix-lib').execution.txHelper
  ...
  constructor (opt, executionContext) {
    ...
    opt.event.udapp.register('callExecuted', (error, from, to, data, lookupOnly, txResult) => {
      ...
      this._resolveTx(call, call, (error, resolvedData) => {
        console.log('resolveTxCall', error)
        if (!error) {
          try {
            this.log(call, null)
          } catch(e) {
  ...
  log (tx, receipt) {
    ...
    if (resolvedTransaction) {
      ...
      global.eventsDecoder.parseLogs(tx, resolvedTransaction.contractName, compiledContracts, (error, logs) => {
        if (!error) {
          global.terminal.logTx(JSON.stringify({
      ...
      this._api.resolveReceipt(tx, (error, receipt) => {
        ...
        this._resolveTx(tx, receipt, (error, resolvedData) => {
          ...
          if (resolvedData) {
            ...
            global.terminal.logTx(JSON.stringify({tx, receipt, resolvedData}))
          }

          try {
            this.log(tx, receipt)
          } catch(e) {}
          // global.terminal.logTx({tx, receipt})
