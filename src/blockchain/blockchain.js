...
import executionContext from './wrapped-execution-context'
...
// const executionContext = require('./wrapped-execution-context')
...
const InjectedProvider = require('./providers/wrapped-injected')
...
class Blockchain {
  ...
  fromPeb (value, doTypeConversion, unit) {
    if (doTypeConversion) {
      return Caver.utils.fromPeb(typeConversion.toInt(value), unit || 'KLAY')
    }
    return Caver.utils.fromPeb(value.toString(10), unit || 'KLAY')
  }

  toPeb (value, unit) {
    return Caver.utils.toPeb(value, unit || 'gPeb')
  }

  calculateFee (gas, gasPrice, unit) {
    return Caver.utils.toBN(gas).mul(Caver.utils.toBN(Caver.utils.toPeb(gasPrice.toString(10), unit || 'gPeb')))
  }
  ...
  determineGasFees (tx) {
    const determineGasFeesCb = (gasPrice, cb) => {
      ...
      try {
        ...
        txFeeText = ' ' + this.fromPeb(fee, false, 'ether') + ' KLAY'
        priceStatus = true
      } catch (e) {
  ...
  createVMAccount (newAccount) {
    if (this.getProvider() !== 'vm') {
      throw new Error('plugin API does not allow creating a new account through caver connection. Only vm mode is allowed')
    }
